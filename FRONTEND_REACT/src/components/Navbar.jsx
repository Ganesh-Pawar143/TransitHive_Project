      ...
      <nav className="navbar navbar-expand-lg" id='nav_color'>
        <div className="container-fluid d-flex justify-content-between align-items-center">
          <a className="navbar-brand fw-bold fs-3 text-white" href="/">
            <h3 id="logo_nav">Transit Hive</h3>
          </a>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={handleCloseModal}>
          <div className="modal-container" onClick={(e) => e.stopPropagation()}>
            ...
            <h3>Login as</h3>
            <button className="btn btn-primary my-2" onClick={() => navigate('/login/admin')}>ADMIN</button>
            <button className="btn btn-success my-2" onClick={() => navigate('/login/vendor')}>SERVICE PROVIDER</button>
            <button className="btn btn-info my-2" onClick={() => navigate('/login/user')}>CUSTOMER</button>
          </div>
        </div>
      )}
    </div>
